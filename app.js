...
const itemGallery = createItemGallery(galleryItems);
refs.listGallery.insertAdjacentHTML("beforeend", itemGallery);

refs.listGallery.addEventListener("click", onClick);
...
function onClick(event) {
  ...
  galleryItems.forEach(({ preview, original, description }) => {
    if (event.target.src === preview) {
      refs.image.src = `${original}`;
      refs.image.alt = `${description}`;
    }
  });
}
